...
def test_maintenance_skill_is_manual_and_routes_to_one_reference():
    ...
    distill = (skill_dir / "references" / "distill.md").read_text(encoding="utf-8")
    ...
    assert "不要因普通的“沉淀记忆”“整理记忆”或“归档分支”等自然语言自动触发" in skill
    assert "只读取当前类型对应的 reference" in skill
    assert "references/distill.md" in skill
    assert "references/tidy.md" in skill
    assert "references/archive.md" in skill
    assert "第一次调用和后续增量沉淀使用同一套规则" in distill
    assert "用户未确认时保持只读" in distill
    assert "distill 没有 `completed` 状态" in distill
...
def test_distill_print_prompt_is_repeatable_and_review_gated(tmp_path):
    repo = tmp_path / "repo"
    repo.mkdir()
    result = run_cli(
        "maintain",
    ...
    assert "维护模式：distill" in result.stdout
    assert "维护范围：branch+repo" in result.stdout
    assert "第一次调用和后续增量沉淀使用同一套规则" in result.stdout
    assert "在向用户展示本批 proposal 并获得确认前，不执行任何语义记忆写入" in result.stdout
    assert "capture rewrite-entry" in result.stdout
...
def test_distill_rejects_invalid_scope(tmp_path):
    result = run_cli(
        "maintain",
        "distill",
        "--repo",
    ...
    assert result.returncode != 0
    assert "unsupported distill scope" in result.stderr
