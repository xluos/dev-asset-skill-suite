...
```bash
npx dev-memory context sync \
  [--repo <repo-path>] [--branch <branch-name>]
```

### Step 3: 缺内容 → 转 capture

context 只读不写。发现记忆缺关键前提 / 有明显错漏 / 用户提供了新信息，走 `dev-memory-capture` 写入。

## 非 git 模式

cwd 不是 git repo 且没有子 git repo 时：
- 记忆按 `.dev-memory-id` dotfile 定位到 repo 共享层
