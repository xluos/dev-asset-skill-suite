...
| 当前情境 | 走哪个 skill |
| --- | --- |
| 用户说"分支改名了"/"基于 X 接力开发"/"把上个分支的记忆搬过来"/"换分支但是同一条线" | 引导跑 `dev-memory branch`（CLI，交互式选择来源/目标 + 冲突处理） |
...
不走 skill，走 CLI：`dev-memory branch`。无参数 = 交互式，按当前分支状态分流：

- **当前分支已有记忆** → 选 rename / fork / init / delete
- **当前分支空骨架或未初始化** → 选 fork / rename 从某个已有分支接力

候选列表自动来自 `git branch` ∪ 已存在的记忆目录，每项标注"git / 无记忆 / 空骨架 / N 条记忆 / 最近更新时间"，支持 type-ahead 过滤。所有破坏性操作（rename / fork 覆盖、delete、init）默认推荐"备份后执行"（移到 `_archived/<branch>-<UTC>/`），`--force` 必须显式。

fork / rename 完成后会自动：1) 把 5 个 markdown 里的"## 分支"自我标识改写为新分支名；2) 把 progress.md 的 auto-sync 区重置为占位（让下次 capture sync-working-tree 重生成）；3) 在 overview.md 顶部加"## 分支起源" section 记录来源分支与时间。用户自由文本里出现的源分支名故意不动。
