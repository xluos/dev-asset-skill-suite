...
- **首次整理入口**：`dev-assets-setup`（不再是前置门禁，而是把 unsorted.md 合并到结构化文件）
- **定期校准入口**：`dev-assets-tidy`（已结构化的条目随时间漂移时，浏览器审阅 keep/delete/edit）
...
| 用户说"整理记忆"/"清下过期的"/"哪些还成立"/"记忆有重复或不对的"；或长期主分支记忆累积、context 看起来含 v1 残留/模板占位/stale 信号 | `dev-assets-tidy` |
| 用户说"整理 dev-assets"/"初始化 dev-assets"；或 unsorted.md 累积 ≥20 条；或 setup_completed=false 且即将产生新稳定结论 | `dev-assets-setup` |
